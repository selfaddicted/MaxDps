...
function Custom:OnEnable()
	self.CustomRotations = self.CustomRotations and wipe(self.CustomRotations) or {}
	self.Specs = self.Specs and wipe(self.Specs) or {}
	-- private for dropdowns
	self.classList = self.classList and wipe(self.classList) or {}
	self.specList = self.specList and wipe(self.specList) or {}

	for i = 1, GetNumClasses() do
		local classDisplayName, classTag, classId = GetClassInfo(i)
		tinsert(self.classList, {
		...
		for sI = 1, specNum do
			...
			self.Specs[classId] = self.Specs[classId] or {}
			self.Specs[classId][sI] = specName
			self.specList[classId] = self.specList[classId] or {}
...
function Custom:LoadCustomRotations()
	...
	for _, rotation in pairs(MaxDps.db.global.customRotations) do
		if rotation.enabled and rotation.class ~= nil and rotation.spec ~= nil then
			...
			self.CustomRotations[rotation.class] = self.CustomRotations[rotation.class] or {}
